...
userRouter.post("/generate-api", UserMiddleware, async (req, res) => {
  try {
...
userRouter.get("/get-api", UserMiddleware, async (req, res) => {
  try {
    const UserId = req.userId;
...
userRouter.delete("/delete-key/:id", UserMiddleware, async (req, res) => {
  try {
    const apiId = req.params.id;
    if (!apiId) {
      return res.status(402).json({
        message: "Api Key Id Require",
        success: false,
      });
    }
    ...
    if (deleteApiKey) {
      return res.status(201).json({
        message: "Api Key Invoked From Your Account",
        success: false,
      });
    }

    return res.status(402).json({
      message: "Errror While Deleting Api Entry",
      success: false,
    });
  } catch (e) {
    console.log(e);
...
userRouter.get("/get-user", UserMiddleware, async (req, res) => {
  try {
    const userId = req.userId;
